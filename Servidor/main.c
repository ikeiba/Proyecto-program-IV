...
int main(int argc, char *argv[]) {
    ...
    registrarMensaje("Initialising Winsock...\n");
	if (WSAStartup(MAKEWORD(2, 2), &wsaData) != 0) {
		printf("Failed. Error Code : %d", WSAGetLastError());
        registrarMensaje("Failed. Error Code : %d", WSAGetLastError());
		return -1;
	}

	printf("Initialised.\n");
    registrarMensaje("Initialised.\n");

	//SOCKET creation
	if ((conn_socket = socket(AF_INET, SOCK_STREAM, 0)) == INVALID_SOCKET) {
		printf("Could not create socket : %d", WSAGetLastError());
        registrarMensaje("Could not create socket : %d", WSAGetLastError());
		WSACleanup();
		return -1;
	}

	printf("Socket created.\n");
    registrarMensaje("Socket created.\n");
	...
	if (bind(conn_socket, (struct sockaddr*) &server,
        ...
        registrarMensaje("Bind failed with error code: %d", WSAGetLastError());
    ...
    do{
        ...
        if (listen(conn_socket, 1) == SOCKET_ERROR) {
            ...
            registrarMensaje("Listen failed with error code: %d", WSAGetLastError());
            closesocket(conn_socket);
            WSACleanup();
            return -1;
        }
        ...
        registrarMensaje("Waiting for incoming connections...\n");
        int stsize = sizeof(struct sockaddr);
        comm_socket = accept(conn_socket, (struct sockaddr*) &client, &stsize);
        // Using comm_socket is able to send/receive data to/from connected client
        if (comm_socket == INVALID_SOCKET) {
            printf("accept failed with error code : %d", WSAGetLastError());
            registrarMensaje("accept failed with error code : %d", WSAGetLastError());
            closesocket(conn_socket);
            WSACleanup();
            return -1;
        }
        ...
        registrarMensaje("Incomming connection from: %s (%d)\n", inet_ntoa(client.sin_addr),
                ntohs(client.sin_port));
        // Closing the listening sockets (is not going to be used anymore)

        //SEND and RECEIVE data
        printf("Waiting for incoming messages from client... \n");
        registrarMensaje("Waiting for incoming messages from client... \n");
        do {
